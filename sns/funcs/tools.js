...
function stepsLeftRight(JCubees, currentMeshes) {
	...
	for(var mesh in JCubees) {
		...
		if(!(Z in blockers[Y])) {
			blockers[Y][Z] = [30 - 21*60, 30 + 20*60]; // x barrier by left hand grid on the outside, x barrier by right hand grid on outside;
		}
...
function stepsUpDown(JCubees, currentMeshes) {
	...
	for(var mesh in JCubees) {
		...
		if(!(X in blockers[Z])) {
			blockers[Z][X] = [-30, 30 + 25*60]; // y barrier by bottom grid on the outside, y barrier by top grid on outside;
		}
...
function stepsForwardBack(JCubees, currentMeshes) {
	...
	for(var mesh in JCubees) {
		...
		if(!(Y in blockers[X])) {
			blockers[X][Y] = [30 - 60*22, 30 + 21*60]; // y barrier by bottom grid on the outside, y barrier by top grid on outside;
		}
